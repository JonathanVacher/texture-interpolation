...
class TexWasserModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.spectrum_constraint = opt.spectrum_constraint
        self.target_ps_1 = utils.power_spectrum(self.input_tex_1)
        self.target_ps_2 = utils.power_spectrum(self.input_tex_2)
    ...
    def get_net_and_loss(self):
        ...
        for layer in self.cnn.children():
            ...
            if name in self.tex_layers:
                ...
                if self.interp_method == '1':
                    tex_loss_1 = utils.TexLossOptTrans(target_feature_1, self.device)
                    tex_loss_2 = utils.TexLossOptTrans(target_feature_2, self.device)
                elif self.interp_method == '2':
                    tex_loss_1 = utils.TexLossOptTransAlternative(
                                    target_feature_1, target_feature_2, 
                                    self.weight_1, self.device)
                    tex_loss_2 = utils.TexLossOptTransAlternative(
                                    target_feature_1, target_feature_2, 
                                    self.weight_1, self.device)
        ...
        # trim off the layers after the last tex losses
        for i in range(len(self.model) - 1, -1, -1):
            if isinstance(self.model[i], utils.TexLossOptTrans):
                break
            if isinstance(self.model[i], utils.TexLossOptTransAlternative):
                break
    ...
    def optimize_parameters(self):
        ...
        def closure():
            ...
            for i in range(len(self.tex_losses_1)):
                ...
                self.tex_score_1 += self.spectrum_constraint\
                    *utils.power_spectrum_constraint(self.output_tex,
                                                     self.target_ps_1)
                self.tex_score_2 += self.tex_losses_2[i].loss
                self.tex_score_2 += self.spectrum_constraint\
                    *utils.power_spectrum_constraint(self.output_tex,
                                                     self.target_ps_2)
